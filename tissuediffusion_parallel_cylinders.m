function [traj, headerinfo, stats] = tissuediffusion_parallel_cylinders(tissue, numwalkers, max_t, dt, initialisation)
    ...
    switch initialisation
        ...
        case 'random_quartile'
            ...
            elseif(chance >= tissue.ficvf && chance < (tissue.ficvf + tissue.fee))                
                ...
                index = ceil(rand * length(tissue.vtree.X)); %Introduce random seed - BJ 180716
                walker.x = tissue.vtree.X(index);
                walker.y = tissue.vtree.Y(index);
                walker.z = tissue.vtree.Z(index);
                walker.lab = 2;
        ...
        case 'cylinders'
            index = ceil(rand * length(tissue.cyl_centres));
            walker.x = tissue.cyl_centres(index,1);
            walker.y = tissue.cyl_centres(index,2);
            walker.z = (rand * (tissue.dimz/2)) + (tissue.dimz/4);
            walker.cylID = index;
            walker.lab = 4;
...
%             end
        elseif(walker.lab == 4)
            cyl_centre = tissue.cyl_centres(walker.cylID,:);
            next_pos = [next_x, next_y];
            dist = pdist([cyl_centre; next_pos], 'euclidean');
            if(dist > tissue.cyl_rads(walker.cylID))
                thit = 1;
            end
        end
    end
...
%Complete one full step
    ...
    function [walker] = isintra(walker,tissue)
        ...
        if(walker.lab == 2)
            ...
            walker.cellid = NaN;
        elseif(d <= tissue.rads(k) && walker.lab~=4)
